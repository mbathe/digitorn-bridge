...
| `/api/discovery/prompt-preview` | POST | Preview a prompt with variables |
| `/api/discovery/generate-package-manifest` | POST | Auto-gen package.toml from YAML |
...
| `/api/builder/drafts/{id}/deploy` | POST | Compile + deploy |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/admin/credentials` | GET/POST | List / create system credentials |
| `/api/admin/credentials/{cid}` | DELETE | Delete system credential |
| `/api/admin/quotas` | GET/POST | List / upsert system quotas |
| `/api/admin/quotas/{qid}` | DELETE | Delete system quota |
...
| `/api/modules/{mid}/execute` | POST | Execute a module action directly (admin) |
...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/config` | GET | Current effective config |
| `/api/config` | PATCH | Update runtime-tunable fields |
| `/api/config/browse` | GET | Browse directories (file picker helper) |

## Security Profiles

Per-app security profiles (capabilities, grants, denials).
