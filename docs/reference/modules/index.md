...
Each module exposes a set of `@action`-decorated methods that
...
and other agentic IDEs. The mapping is centralised in
`packages/digitorn/core/runtime/tool_names.py`.

When restricting per-agent module access ([Agents - Per-agent
