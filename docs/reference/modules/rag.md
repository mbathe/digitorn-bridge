...
|---------|---------|--------|
| Embedding | `minilm-l12` (384 d, multilingual, 220 MB) | `embeddings.py` |
| Backend | Qdrant in-memory | `config.py` |
| Strategy | Hybrid (BM25 + semantic + RRF) | `config.py` |
| Chunking | recursive, 500 chars, 50 overlap | `config.py` |
| Cache | enabled, in-memory, 1 h TTL | `config.py` |
