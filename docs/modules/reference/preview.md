...
Socket.IO. Powers zero-code live previews for digitorn-builder,
React sandboxes, slides, timelines, YAML panels, and any
workflow editor / multi-agent orchestrator.

| Property | Value | Source |
|----------|-------|--------|
| Module id | `preview` | `module.py` |
| Action count | 17 (all `internal=True`) | `module.py` |
...
```
┌───────┐  Python   ┌──────────┐  Socket.IO   ┌────────┐
│ Agent │ ────────▶ │workspace │ ──▶ preview ──────▶ web │
│       │ (WsWrite) │  module  │      (bus)         │ UI │
└───────┘           └──────────┘                    └────┘
```
...
- API integration (Socket.IO defaults, room subscription):
  [API Integration → Real-time](../../app-language/14-api-integration.md#real-time-socketio)
