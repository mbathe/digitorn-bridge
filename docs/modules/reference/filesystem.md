...
|----------|-------|--------|
| Module id | `filesystem` | `module.py` |
| Version | `2.0.0` | `module.py` |
| Action count | 5 | |
| Type | system | |
...
1. **Minimal visible params** → fewer LLM mistakes. All
   ...
   to the model.
2. **Powerful implementation** → hidden params + smart
   defaults handle encoding, fuzzy matching, recovery.
...
> unread files larger than 500 bytes. Files <500 bytes can
...
- Behavior engine (built-in `read_before_edit` rule):
  [Behavior Engine](../../app-language/43-behavior.md#the-14-built-in-rules)
- Sandbox Landlock guards (kernel-level path enforcement):
  [OS-Level Sandbox → Landlock](../../app-language/35-sandbox.md#1-landlock--filesystem)
