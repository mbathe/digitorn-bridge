...
```
Client Runtime                          Daemon
     │                                    │
     │──── WebSocket Connect ────────────>│
     │                                    │
     │──── MSG: register ────────────────>│  Runtime declares capabilities
     │<─── MSG: registered ──────────────│  Daemon accepts, sends session config
     │                                    │
     │<─── MSG: execute ─────────────────│  Daemon sends action to execute
     │──── MSG: result ──────────────────>│  Runtime returns result
     │                                    │
     │<─── MSG: execute ─────────────────│  (repeat for each tool call)
     │──── MSG: result ──────────────────>│
     │                                    │
     │──── MSG: heartbeat ───────────────>│  Every 15s
     │<─── MSG: heartbeat ──────────────│
     │                                    │
     │──── MSG: disconnect ──────────────>│  Clean shutdown
     │                                    │
```
...
```
Daemon:  execute(req_1, filesystem.read)
Daemon:  execute(req_2, filesystem.glob)
Daemon:  execute(req_3, shell.bash)
Runtime: result(req_2, [...files])        ← glob finishes first
Runtime: result(req_1, {content: "..."})  ← read finishes second
Runtime: result(req_3, {stdout: "..."})   ← bash finishes last
```
