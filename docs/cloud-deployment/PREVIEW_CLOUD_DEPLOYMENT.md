...
```
                         Internet
                            ...
                            ▼
  ┌─────────────────────────────────────────────────┐
  │   *.preview.digitorn.cloud  ─►  Edge proxy      │   wildcard DNS
  │   (nginx / Caddy / Traefik)                     │   + TLS
  │   ─ extracts {port} from subdomain              │
  ...
  └─────────────────────────────────────────────────┘
       ...
       │                    │                    │
       ▼                    ▼                    ▼
  127.0.0.1:3001       127.0.0.1:5173       127.0.0.1:8001
  (agent's Vite)       (agent's Next dev)   (agent's API)
       │                    │                    │
   spawned by           spawned by           spawned by
   session A's          session B's          session A's
   agent via Bash       agent via Bash       agent via Bash
```
...
That's it. The daemon resolves attachments to URLs using the
template; the edge proxy resolves URLs to loopback ports.
...
echo "==> Daemon health"
curl -fsS "$DAEMON_HOST/healthz" | jq .

echo
echo "==> Web-preview module health"
curl -fsS "$DAEMON_HOST/health/web_preview" | jq .
...
- The nginx config above doesn't add auth. If you want only logged-in
  users to reach previews, add `auth_request` directive pointing at
  the daemon's `/auth/verify`.
