...
(`packages/digitorn/core/app/schema.py`, `extra: forbid`). An
...
|-------|------|---------|--------|
| `id` | string | *required* | `schema.py`. Unique slug within the app. |
| `role` | string | `"worker"` | `schema.py`. Functional roles drive runtime behavior: `coordinator` (can spawn agents), `specialist` (pre-configured expert), `worker`. Free-form descriptive roles (`assistant`, `analyst`, `reviewer`, ...) are also accepted and surfaced in the system prompt. |
| `brain` | AgentBrain | *required* | `schema.py`. See [Brain configuration](#brain-configuration) below. |
| `system_prompt` | string | `""` | `schema.py`. Injected at conversation start. Supports `{{...}}` templates. |
| `plan_first` | bool | `true` | `schema.py`. When true, the agent must explain its plan in plain text before executing tools on the first turn. Prevents silent tool calls. |
| `specialty` | string | `""` | `schema.py`. Short description of the specialist's expertise; shown to coordinators in the spawn catalog. |
| `delegate_to` | list[string] | `[]` | `schema.py`. Agent IDs this coordinator can delegate to. The compiler verifies each entry references a declared agent id. |
| `skills` | string | `""` | `schema.py`. Path to a `.md` file with detailed methodology / instructions for this specialist. |
| `capabilities` | list[string] | `[]` | `schema.py`. Names of skill files to auto-load from the bundle's `skills/` directory. The compiler reads `skills/<name>.md` and appends the content under an `## Available capabilities` section in the system prompt. |
| `modules` | list[string \| dict] | `[]` | `schema.py`. Per-agent module restriction. See [Per-agent module access](#per-agent-module-access) below. Empty = inherits from the coordinator. |
| `pool` | AgentPoolConfig | default-instance | `schema.py`. See [Coordinator pool](#coordinator-pool). |
| `coordination` | CoordinationBlock\|None | `null` | `schema.py`. Phase-9 grouped block (`delegate_to` + `pool`). Aliased into the legacy fields at compile. |
| `instructions` | InstructionsBlock\|None | `null` | `schema.py`. Phase-9 grouped block (`file` + `capabilities` + `specialty`). Aliased into the legacy fields at compile. |
| `hooks` | list[HookConfig] | `[]` | `schema.py`. Per-agent hooks merged with `runtime.hooks` but only evaluated when this agent is active. Use for specialist-specific behavior. App-wide hooks still fire for every agent; these add on top. |
...
| Field | Type | Default | Source |
|-------|------|---------|--------|
| `provider_id` | string\|None | `null` | `schema.py`. Reference to a named provider. If set, `provider`/`model`/`config` are ignored. |
| `provider` | string\|None | `null` | `schema.py`. Provider hint, validated against the known set. |
| `model` | string\|None | `null` | `schema.py`. Model identifier (e.g. `deepseek-chat`, `claude-sonnet-4-5`). |
| `backend` | `openai_compat | anthropic` | `openai_compat` | `schema.py`. Wire protocol. |
| `config` | dict | `{}` | `schema.py`. Provider-specific config (api_key, base_url, ...). |
| `credential` | string \| dict \| null | `null` | `schema.py`. Recommended over inline `{{secret.X}}`. Compact: `credential: openai_main`. Explicit: `credential: { ref: openai_main, scope: per_user }`. The runtime resolves at activation and injects into `config`. |
| `temperature` | float\|None | `null` | `schema.py` |
| `max_tokens` | int\|None | `null` | `schema.py` |
| `top_p` | float\|None | `null` | `schema.py` |
| `timeout` | float\|None | `null` | `schema.py`. Seconds. |
| `native_tool_use` | bool\|None | `null` (auto-detect) | `schema.py`. Override native tool-calling detection. See [Native vs text-based tool calling](#native-vs-text-based-tool-calling). |
| `context` | ContextConfig\|None | `null` | `schema.py`. Per-brain context override. Inherits from `runtime.context` if not set. |
| `fallback` | AgentBrain\|None | `null` | `schema.py`. Used when the primary returns a billing / credit error (HTTP 402, "Insufficient Balance"). Switches back on the next turn. |
| `vision` | bool\|None | `null` (auto) | `schema.py`. Whether the model supports image input. `null` = auto-detect from model name; `true`/`false` to force. |
| `image_generation` | bool | `false` | `schema.py` |
| `image_detail` | string | `"auto"` | `schema.py`. `auto` / `low` (512 px, cheaper) / `high` (native, more accurate). |
| `max_images_per_turn` | int [0, 100] | `5` | `schema.py`. `0` = unlimited. |
