...
`packages/digitorn/core/cli/mcp_cli.py` `mcp_cli` (Typer
sub-command, registered in `core/server.py`). Seven commands:
...
Three top-level fields (`schema.py`):
...
(`oauth.py+`); each entry is a `{ authorize_url, token_url,
