...
optional ``schema_version`` (`AppDefinition` in
...
> `digitorn yaml migrate-v2 path/to/app.yaml` (CLI command in
> `core/cli/yaml_migrate.py`). The compiler keeps accepting legacy
> YAMLs (`execution:`, `modules:` at the top level, ...) by
...
`AppMeta` (`schema.py`, `extra: forbid`).
...
|-------|------|---------|-------|
| `app_id` | string | *required* | Unique app id (slug). |
| `name` | string | *required* | Human-readable name. |
| `version` | string | `"1.0"` | |
| `schema_version` | string | `"1"` | YAML schema version (forward-compat). |
| `description` | string | `""` | |
| `author` | string | `""` | |
| `tags` | list[string] | `[]` | Searchable tags. |
| `icon` | string | `""` | Emoji, icon name, URL, or `data:` URI. Empty → client generates a colored circle from `app_id`. |
| `color` | string | `""` | Hex format `#8B5CF6`. Empty → auto-derived from `app_id` hash. |
| `category` | string | `"general"` | Used by clients for catalog filtering. Examples: `coding`, `writing`, `research`, `data`, `devops`, `design`, `communication`, `automation`, `general`. |
| `quick_prompts` | list[QuickPrompt] | `[]` | Mirror of `ui.quick_prompts`. |
...
`RuntimeBlock` (`schema.py`, `extra: forbid`). Every field that
...
| `max_sessions_per_user` | int ≥ 0 | `10` | `0` = unlimited. |
| `max_concurrent_activations` | int ≥ 1 | `20` | Throttle parallel LLM calls when a broadcast trigger fires. |
| `workdir` | string | `""` | Renamed from legacy `execution.workspace`. Disambiguates from `ui.workspace` (renderer). |
...
| `context` | ContextConfig | default-instance | See [`runtime.context`](#runtimecontext--context-window-management) below. |
| `hooks` | list[HookConfig] | `[]` | See [Tool Hooks](31-tool-hooks.md). |
| `watchers` | bool | `false` | |
| `scheduler` | bool | `false` | Requires `watchers: true`. |
| `default_channel` | string | `"llm_notification"` | |
| `middleware` | list[dict] | `[]` | See [Middleware](17-middleware.md). |
| `pipeline` | list[PipelineStep] | `[]` | `mode=pipeline` only. |
| `triggers` | list[TriggerConfig] | `[]` | See [Triggers](09-triggers.md). |
| `input`, `output` | InputConfig, OutputConfig | default-instances | `mode=one_shot` only. |
| `payload_schema` | PayloadSchemaConfig\|None | `null` | `mode=background` only. |

### `runtime.context` — Context window management

`ContextConfig` (`schema.py`, `extra: forbid`). Eight fields:
...
| `strategy` | `truncate \| summarize` | `summarize` | |
...
`AgentDefinition` (`schema.py`, list-shape, `extra: forbid`). Full
...
`ToolsBlock` (`schema.py`, `extra: forbid`).
...
Map of module-id → `ModuleBlock`. Each `ModuleBlock` has 5 fields
(`schema.py`, `extra: forbid`):

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `config` | dict | `{}` | Static config pushed via `module.on_config_update(config)` at bootstrap. Validated against the module's `CONFIG_MODEL` if declared. |
| `setup` | list[SetupStep] | `[]` | Ordered actions executed at bootstrap. Each step = `{action: str, params: dict}`. |
| `constraints` | dict | `{}` | Universal: `allowed_actions`, `blocked_actions`. Module-specific keys validated against the module's `ConstraintSpec`. |
...
`CapabilitiesConfig` (`schema.py`, `extra: forbid`). Optional
(`null` = dev/test mode, no enforcement). When present:

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `default_policy` | `auto \| approve \| block` | `approve` | |
| `max_risk_level` | `low \| medium \| high` | `medium` | |
...
Map of channel-instance-name → `ChannelInstanceConfig`. See
[Channels (Bidirectional I/O)](40-channels.md) for the full surface.

## `security:` — Runtime boundaries

`SecurityBlock` (`schema.py`, `extra: forbid`). All three sub-fields
are optional.
...
| Field | Type | Doc |
|-------|------|-----|
| `behavior` | BehaviorConfig\|None | [Behavior Engine](43-behavior.md) |
| `sandbox` | SandboxConfig\|None | [OS Sandbox](35-sandbox.md) |
| `credentials_schema` | CredentialsSchemaConfig\|None | [Credentials](../reference/runtime/credentials.md) |
...
`DevBlock` (`schema.py`, `extra: forbid`).
...
`AppDefinition.flow` (FlowConfig | None, default `null`). Promoted
to a **top-level block** in v2 because flow is a paradigm shift:
explicit scenography (a directed graph of nodes with conditional
edges) replaces implicit `Agent()`-tool coordination.

`FlowConfig` is defined in `core/app/flow.py` (`extra: forbid`).
...
string in the YAML (entry point `resolve_variables` in
`packages/digitorn/core/app/variables.py`). Six namespaces, each
with a fixed resolution time.
...
If the left side fails to resolve, the right side is used. Works
with any namespace.
...
Resolved at compile time from `_SYS_VARIABLES` in `variables.py`.
The full list:
...
resolve to file content / URLs at compile time (resolvers
`_resolve_prompt`, `_resolve_skill`, etc. in `variables.py`):
...
This example deploys cleanly against a fresh daemon and exercises
each of the eight blocks. The brain uses a local Ollama model so
no external credentials are required.

```yaml
app:
  app_id: example-canonical
  name: "Canonical 8-block Example"
  version: "1.0"
  category: general
  description: "Reference YAML covering every top-level block."
...
dev:
  variables:
    note: "{{app.id}}-built-on-{{sys.date}}"
```

The example is verified end-to-end: it deploys via
`digitorn dev deploy -d <daemon> example.yaml` and a `ping`
prompt returns a single short reply through the Socket.IO
`message_done` event.
