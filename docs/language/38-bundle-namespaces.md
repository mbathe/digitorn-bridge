...
|---------|--------|-------------|--------|
| `{{prompt.X}}` | `prompts/X.md` | File content (raw markdown) | `variables.py` `_resolve_prompt` |
| `{{skill.X}}` | `skills/X.md` | File content | `variables.py` `_resolve_skill` |
| `{{behavior.X}}` | `behavior/X.yaml` | Parsed YAML, returned as a JSON string | `variables.py` `_resolve_behavior` |
| `{{asset.X}}` | `assets/X` | URL: `/api/apps/<app_id>/assets/assets/X` | `variables.py` `_resolve_asset` (called from `_lookup:355`) |
| `{{asset_b64.X}}` | `assets/X` | `data:<mime>;base64,<payload>` URI | `variables.py` `_resolve_asset_b64` |
| `{{include:path}}` | `<bundle>/path` | Parsed YAML fragment inlined into the parent structure | `variables.py` `_resolve_include` |
...
| `widgets/*.yaml` | Merged into `ui.widgets.inline` (key = file stem) | `WidgetsConfig` docstring (`schema.py`) |
