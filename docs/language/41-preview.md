...
|-------|---------|--------|
| `ui.workspace` | **Renderer for in-memory virtual files** the agent writes via `WsWrite/Read/Edit/Glob/Grep/Delete`. The client picks the right viewer (React, LaTeX, slides, code, ...) based on `render_mode`. | `schema.py` `WorkspaceBlock` + `modules/workspace/module.py` |
...
|-------------|-----|--------|
| `WsWrite` | `workspace.write` | `module.py` |
| `WsRead` | `workspace.read` | `module.py` |
| `WsEdit` | `workspace.edit` | `module.py` |
| `WsGlob` | `workspace.glob` | `module.py` |
| `WsGrep` | `workspace.grep` | `module.py` |
| `WsDelete` | `workspace.delete` | `module.py` |
