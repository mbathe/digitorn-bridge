...
| Everything else | None | No bucket — there is **no catch-all** (removed in `server.py` because legitimate Flutter polling kept hitting 429). |
...
`digitorn_auth` package, registered at `server.py`) requires
...
### Layer 2 — `/healthz` lightweight liveness probe

Designed to be called every few seconds by container orchestrators,
load balancers, and the supervisor below. Cheap (no psutil), returns
...
Routes consumed by external probes:

- `/healthz` — fast liveness (this section)
- `/health` — full system info (CPU, memory, threads, worker pool stats)
- `/health/web_preview` — operational metrics for the web_preview module
...
When the threshold trips, the daemon logs:

```text
zombie_poll_detected ip=127.0.0.1 path=/api/apps/X/web-preview
(threshold reached at 30 404s/60s window) - returning 429 to subsequent requests
```
...
| `/healthz` unreachable | 5 consecutive failures (50s) | `--max-failures` |
| `/healthz` `degraded` for 60s | 6 consecutive probes | `--max-degraded` |
...
last 200 lines of daemon stdout, the exit code, the pid, and the
last successful `/healthz` payload. Use it for offline triage:
...
| Multi-node cluster | k8s with horizontal autoscaling + `/healthz` |
...
throttle, and the rich `/healthz` payload.
