...
`packages/digitorn/core/runtime/bootstrap.py` `_choose_tool_injection()`.
...
| Action | Source |
|--------|--------|
| `search_tools` | `actions_meta.py` |
| `get_tool` | `actions_meta.py` |
| `execute_tool` | `actions_meta.py` |
| `list_categories` | `actions_meta.py` |
| `browse_category` | `actions_meta.py` |
| `run_parallel` | `actions_meta.py` |
| `use_skill` | `actions_meta.py` |
| `call_app` | `actions_meta.py` |
| `ask_user` | `actions_meta.py` |

**Always direct (background primitive)**:

| Action | Source |
|--------|--------|
| `background_run` | `actions_background.py`. Single action with five modes via params: launch (`name`+`params`), status check (`task_id`), cancel (`task_id`+`cancel=true`), wait (`task_id`+`wait=true`+`timeout`), list all (`list_tasks=true`). |
