...
(`packages/digitorn/core/app/schema.py`) and the daemon runs it.
...
(entry point `digitorn = "digitorn.core.server:main"`). All
sub-commands are registered in `core/server.py` via Typer.
