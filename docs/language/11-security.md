...
|------|------------|-------------------|--------|
| 0 | `gate0_inactive` | The app is deployed but not active. Admin profiles bypass. | `security.py` |
| 1a | `gate1_module` | The agent's profile can't access the module (`hidden_modules` or per-agent `modules` restriction filters it out). | `security.py` |
| 1b | `gate1_hidden` | The action is in `hidden_actions` for this module. | `security.py` |
| 2 | `gate2_risk` | Action's declared risk exceeds `max_risk_level`, with no explicit grant or per-action policy. | `security.py` |
| 3 | `gate3_permissions` | Action declares `required_permissions` (symbolic, e.g. `fs.write`, `net.http`) and the profile lacks them. | `security.py` |
| 4 | `gate4_policy` | Resolved action policy is `block` (denied) or `approve` (paused for HITL). | `security.py` |
| 5 | `gate5_classification` | Per-tool data-classification rule rejects the params (e.g. PII detected in a non-PII-allowed channel). | `security.py` |
| 6 | `gate6_rate_limit` | Per-action rate limit window exceeded. | `security.py` |
