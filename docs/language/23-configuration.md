...
`packages/digitorn/core/config.py`. Settings load from layered
...
| `app` | `AppConfig` | `config.py` | Built-in app behavior (digitorn-chat default app). |
...
`config.py` `DefaultModelConfig`. Used by the auto-deployed
