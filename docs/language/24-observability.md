...
| Path | Source | Purpose |
|------|--------|---------|
| `GET /health` | `server.py` | Rich health probe — version, status, system metrics, event-loop lag, watchdog, worker-pool stats. Status flips to `degraded` when loop lag > 500 ms or the turn pool is saturated. |
| `GET /healthz` | `server.py` | Liveness probe (`{"status": "alive"}`). **Public** — exempt from auth middleware. Used by Kubernetes liveness checks. |
| `GET /readyz` | `server.py` | Readiness probe (`{"status": "draining"}` with HTTP 503 when shutting down, otherwise OK). **Currently requires JWT auth** when `server.auth_enabled=true` — for K8s probes either disable auth, run a sidecar that mints a token, or use `/healthz` instead. |
...
| Path | Source | Format |
|------|--------|--------|
| `GET /api/metrics` | `server.py` | JSON — global summary across all apps and sessions. |
| `GET /api/metrics/prometheus` | `server.py` | Prometheus exposition format — same data as `/api/metrics` but in `# HELP` / `# TYPE` form. |
| `GET /metrics` | `server.py` | Mirror of `/api/metrics/prometheus`. Some Prometheus scrapers hard-code `/metrics`. |
| `GET /api/metrics/sessions` | `server.py` | List of every active session with its metrics. |
| `GET /api/metrics/sessions/{session_id}` | `server.py` | Single-session detail. |
| `GET /api/metrics/apps/{app_id}` | `server.py` | Per-app rollup. |
...
(`session_metrics.py`) tracking real-time numbers.
...
from digitorn.core.runtime.session_metrics import (
    get_session_metrics,        # session_metrics.py
    list_active_metrics,        # session_metrics.py
    app_summary,                # session_metrics.py
    global_summary,             # session_metrics.py
)
...
`packages/digitorn/core/api/modules.py`. Each module decides
...
`packages/digitorn/core/app/channels/registry.py`. Each
...
(`channels/base.py`; the abstract `health_check()` is at
`channels/base.py`). Useful when triggers depend on inbound
...
(`packages/digitorn/core/api/credentials.py`). Returns the
...
(`api/user.py`). Filters: `event_type` (supports trailing
