...
|----------|-------|--------|
| **App-level** | Every LLM call (before / after) | `core/middleware.py` `AppMiddlewarePipeline` |
| **Module-level** | Every tool call to a specific module | `core/middleware.py` `ModuleMiddlewarePipeline` |
...
`core/middleware_store.py` `MiddlewareDescriptor`. Custom
...
The CLI commands live in `core/cli/middleware_cli.py`. After
