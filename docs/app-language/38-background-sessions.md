...
`runtime.session_mode` (`schema.py:2372`).

| Mode | Behaviour |
|------|-----------|
| `mono` (default) | Each user has **exactly one** background session per app. Auto-created on first interaction (`background.py:488` → `store.get_or_create_mono`). |
| `multi` | Each user can create up to `max_sessions_per_user` (`schema.py:2380`, default 10, 0 = unlimited) sessions, each with distinct params, routing keys, workspace. Created via `POST /background-sessions`. |
...
`max_concurrent_activations` (`schema.py:2384`) gates the asyncio
semaphore at `background.py:821` — the daemon won't fire more than
...
(`background_session_store.py:224`):
...
The HTTP loop at `background.py:1394-1418` resolves these
placeholders inside both `message` and `routing_key`:

| Placeholder | Source |
|-------------|--------|
| `{{event.body}}` | Raw request body (truncated to 10 000 bytes for `message`, 200 bytes for `routing_key`). |
| `{{event.path}}` | Request path. |
| `{{event.method}}` | HTTP method. |
| `{{event.header.X-User-Id}}` | Header value (literal name lookup). Recognised: `X-User-Id`, `X-Session-Id`, `X-GitHub-Event`, `X-Gitlab-Event`, `X-Webhook-Event`. |
| `{{event.query.<name>}}` | URL query parameter. |
| `{{event.path}}` (watch) | Matched filesystem path. |
...
`TriggerConfig.type` (`schema.py:1362`) accepts `cron`, `watch`,
`http`. The legacy background runtime starts one of three
asyncio loops per trigger (`background.py:719-731`):
...
> is loaded, `run_background` (`background.py:692-709`) hands
...
`background.py:582` `_TriggerCircuitBreaker`. Pauses a trigger
...
**payload** (`background_session_store.py:295-306`).
...
| File **bytes** | Disk: `~/.digitorn/apps/<app_id>/sessions/<sid>/payload/<safe_name>` (`background_session_store.py:54`). | Daemon at every tick (file size cap **10 MiB**, `background.py:93`). |
...
`background.py:208`), classifies them by MIME, and injects them
...
All under `/api/apps/{app_id}/background-sessions/{sid}/payload`.
Defined in `apps_v2/background.py`:

| Method | Path | Source | Description |
|--------|------|--------|-------------|
| `GET`  | `/payload` | `:351` | Returns `{prompt, metadata, files, validation}`. |
| `PUT`  | `/payload` | `:381` | Sets `prompt` and/or `metadata` (shallow-merged). |
| `POST` | `/payload/files` | `:425` | Multipart upload (server hard cap **25 MiB**, `:449`). |
| `DELETE` | `/payload/files/{filename}` | `:407` | Removes one file (disk + index). |
| `DELETE` | `/payload` | `:395` | Wipes the entire payload. |

## Declarative payload schema

`runtime.payload_schema` (`schema.py:2389`, type
`PayloadSchemaConfig` at `:1768`, `extra: forbid`). Lets an app
...
`PayloadFieldConfig.type` (`schema.py:1492`) accepts six values:
...
`description`, `placeholder` (`schema.py:1488-1526`).

### File rules

`PayloadFileRuleConfig` (`schema.py:1529`):
...
- `payload_schema.required: true` → before each activation the
  ...
  the session if any errors come back (`background.py:849-866`).
...
The compiled schema is included in **every app summary**
returned by the listing routes:
...
So a frontend can render the entire marketplace + every typed
form with a **single round-trip** to `GET /api/apps`.

## Session lifecycle routes

All under `/api/apps/{app_id}/background-sessions`. Defined in
`apps_v2/background.py`:

| Method | Path | Source | Description |
|--------|------|--------|-------------|
| `GET`  | `` | `:313` | List sessions (filtered by JWT identity). |
| `POST` | `` | `:466` | Create a session (mono: get_or_create, multi: enforces `max_sessions_per_user`). |
| `GET`  | `/{sid}` | `:338` | Get one session. |
| `POST` | `/{sid}/pause` | `:516` | Pause — triggers skip this session. |
| `POST` | `/{sid}/resume` | `:529` | Resume. |
| `DELETE` | `/{sid}` | `:542` | Delete. Wipes payload files first (best-effort) so no orphan bytes. |
...
The user_id is read from the JWT
(`request.state.user_id`, `:486`). `params` is opaque; reserved
key `_payload` is managed by the payload routes above.

## Activations

Each trigger fire creates an `Activation` row
(`activation_store.create`, called at `background.py:1035`).
Routes under `/api/apps/{app_id}/`:

| Method | Path | Source | Description |
|--------|------|--------|-------------|
| `GET`  | `/triggers` | `_shared` | List triggers with routing info. |
| `POST` | `/triggers/{trigger_id}/fire` | `_shared` | Manual fire (async). |
| `POST` | `/triggers/{trigger_id}/test` | `_shared` | Sync test with custom payload. |
| `GET`  | `/activations` | `:560` | Paginated history (filterable by `trigger_id`, `status`). |
| `GET`  | `/activations/stats` | `:589` | Aggregated stats (count, success_rate, avg_duration, total tokens). |
| `GET`  | `/activations/{id}` | `:601` | Full activation detail + event timeline. |
| `GET`  | `/errors` | `:615` | Recent failed activations. |

### Activation event timeline

For every activation, an `_ActivationEventRecorder`
(`background.py:379`) wraps the agent's callbacks and writes
...
`background.py:74`: `filesystem.write/edit/create`,
