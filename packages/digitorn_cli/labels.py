...
TOOL_LABELS: dict[str, tuple[str, str | None]] = {
    ...
    # Scheduler
    "schedule_once": ("Scheduling", "label"),
    "schedule_cron": ("Scheduling cron", "label"),
    "schedule_cancel": ("Cancelling job", "job_id"),
    "schedule_list": ("Listing jobs", None),
    "schedule_status": ("Job status", "job_id"),
...
MODULE_ACTION_LABELS: dict[str, tuple[str, str | None]] = {
    ...
    "cache.get_or_set": ("Cache get/set", "key"),
    "cache.delete": ("Cache delete", "key"),
    "cache.delete_by_tags": ("Cache invalidate", "tags"),
    "cache.exists": ("Cache exists", "key"),
    "cache.ttl": ("Cache TTL", "key"),
    "cache.increment": ("Cache incr", "key"),
    "cache.decrement": ("Cache decr", "key"),
    "cache.list_keys": ("Cache keys", "pattern"),
    "cache.stats": ("Cache stats", None),
    "cache.clear": ("Cache clear", None),
    "cache.bulk_get": ("Cache bulk get", None),
    "cache.bulk_set": ("Cache bulk set", None),
    # ── database ──────────────────────────────────────────────────────────────
    # LLM-visible actions
    ...
    # ── cron_native ───────────────────────────────────────────────────────────
    "cron_native.create_schedule": ("Create schedule", "name"),
    "cron_native.update_schedule": ("Update schedule", "schedule_id"),
    "cron_native.delete_schedule": ("Delete schedule", "schedule_id"),
    "cron_native.list_schedules": ("List schedules", None),
    "cron_native.schedule_info": ("Schedule info", "schedule_id"),
    "cron_native.explain_cron": ("Explain cron", "expression"),
    "cron_native.validate_cron": ("Validate cron", "expression"),
    "cron_native.next_runs": ("Next runs", "schedule_id"),
    "cron_native.pause_schedule": ("Pause schedule", "schedule_id"),
    "cron_native.resume_schedule": ("Resume schedule", "schedule_id"),
    "cron_native.run_now": ("Run now", "schedule_id"),
    "cron_native.execution_history": ("Execution history", "schedule_id"),
    "cron_native.add_dependency": ("Add dependency", "schedule_id"),
    "cron_native.remove_dependency": ("Remove dependency", "schedule_id"),
    "cron_native.set_retry_policy": ("Set retry policy", "schedule_id"),
    "cron_native.set_execution_window": ("Set exec window", "schedule_id"),
    "cron_native.add_holiday": ("Add holiday", "date"),
    "cron_native.remove_holiday": ("Remove holiday", "date"),
    "cron_native.list_holidays": ("List holidays", None),
    "cron_native.bulk_create": ("Bulk create schedules", None),
    "cron_native.calendar_view": ("Calendar view", None),
    # ── index ─────────────────────────────────────────────────────────────────
    ...
    # ── pdf ────────────────────────────────────────────────────────────────────
    "pdf.generate": ("Generating PDF", "output_path"),
    "pdf.generate_typst": ("PDF (Typst)", "output_path"),
    "pdf.list_styles": ("PDF styles", None),
    "pdf.read": ("Reading PDF", "path"),
    "pdf.read_tables": ("PDF tables", "path"),
    "pdf.merge": ("Merging PDFs", None),
    "pdf.split": ("Splitting PDF", "path"),
    "pdf.metadata": ("PDF info", "path"),
    # ── spreadsheet (ambiguous names) ─────────────────────────────────────────
    "spreadsheet.create": ("Create spreadsheet", "output_path"),
    "spreadsheet.read": ("Reading Excel", "path"),
    "spreadsheet.info": ("Excel info", "path"),
    "spreadsheet.finalize": ("Finalize Excel", "workbook_id"),
    # ── presentation ──────────────────────────────────────────────────────────
    "presentation.new_presentation": ("New presentation", "title"),
    "presentation.add_slide": ("Add slide", "layout"),
    "presentation.edit_slide": ("Edit slide", "slide_index"),
    "presentation.remove_slide": ("Remove slide", "index"),
    "presentation.reorder_slides": ("Reorder slides", None),
    "presentation.preview_slide": ("Preview slide", "slide_index"),
    "presentation.presentation_state": ("Presentation state", "presentation_id"),
    "presentation.finalize_presentation": ("Finalize presentation", "presentation_id"),
    "presentation.list_presentations": ("List presentations", None),
    "presentation.list_themes": ("List themes", None),
    # ── notebook ──────────────────────────────────────────────────────────────
    "notebook.read": ("Reading notebook", "path"),
    # ── web (ambiguous names) ─────────────────────────────────────────────────
    ...
    # ── hello ─────────────────────────────────────────────────────────────────
    "hello.say_hello": ("Hello", "name"),
    "hello.greet_many": ("Greeting many", None),
    "hello.status": ("Hello status", None),
    # ── shell (FQN labels) ─────────────────────────────────────────────────────
...
def _label_from_registry(
    ...
    try:
        ...
        _reg_mod = _imp("digitorn.modules.registry")
        registry = getattr(_reg_mod, "ModuleRegistry", None)
        if registry is None:
            return None
        registry = getattr(registry, "_instance", None)
        if registry is None:
            return None
...
def _memory_label(name: str, params: dict[str, Any]) -> tuple[str, str] | None:
    ...
    if action == "forget":
        return "Forget", params.get("fact_id", "")

    if action == "add_todo":
        content = params.get("content", "")
        return "+Task", content[:50] + "..." if len(content) > 50 else content

    if action == "update_todo":
        status = params.get("status", "")
        tid = params.get("todo_id", "")
        icons = {"done": ICON_SUCCESS, "in_progress": ">", "blocked": "!", "pending": " "}
...
def _resolve_execute_tool(params: dict[str, Any]) -> tuple[str, str]:
    ...
    # Module-qualified label (e.g., "cache.get" → "Cache get")
