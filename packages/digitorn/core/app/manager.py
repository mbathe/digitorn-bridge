...
class AppManager:
    ...
    def __init__(
        ...
        *,
        ...
        # BUG-080: store the last deploy error per app_id so the
        # /api/apps/{id}/deploy-status route can report ``{deployed,
        # error, traceback}`` instead of leaving the client staring at
        # a 404 forever. Initialise here so ``getattr(manager,
        # "_deploy_errors")`` paths never hit a missing attribute.
        self._deploy_errors: dict[str, dict[str, Any]] = {}
        # Strong references to fire-and-forget asyncio tasks (auto-start
        # background mode apps). Without this, Python GC destroys pending
        # tasks before they start, producing "Task was destroyed but it is
        # pending!" warnings at startup when many apps are reloaded.
        self._bg_start_tasks: set[asyncio.Task] = set()
        self._session_store = SessionStore(
            directory=session_dir,
            backend_url=session_backend_url,
        )
        # Recover any sessions whose objects expired but messages persisted
        ...
        # Accept an externally-provided bus (Socket.IO-backed in the
        # daemon path). Falls back to a local no-sio instance for tests.
    ...
    async def deploy(
        ...
        *,
        ...
        async with self._deploy_lock:
            ...
            # BUG-103: for a first deploy (``previous`` is None), keep
            # the exact pre-BUG-081 flow — no popping, no rollback
            # dance. Only the force=True redeploy path needs the
            # transactional swap, and doing it unconditionally was
            # what caused the post-fix regression on fresh deploys.
            if previous is None:
    ...
    async def _chat_locked(
        ...
        # Always strip {WORKSPACE} — never let the literal placeholder leak
        # to the agent. Fall back to yaml workspace / empty string when the
        # session has no workspace (ws_mode=none).
        if effective_prompt:
        ...
        try:
            ...
            _MAX_EVENTS_PER_TURN = 500  # Safety cap — prevent OOM on runaway turns
        ...
        async def _on_tool_call(name: str, params: dict, result: Any, call_id: str = "") -> None:
            ...
            if action in _MEMORY_ACTIONS:
                await self.event_bus.publish(bus_key, {
                    "type": "memory_update",
                    "data": {"action": action, "result": result_data, "name": name},
                })
                ...
                if _stdout or _stderr:
                    await self.event_bus.publish(bus_key, {
                        ...
                        "data": {"stdout": _stdout[:2000], "stderr": _stderr[:500]},
                    })
        ...
        async def _on_thinking_delta_bus(delta: str) -> None:
            await self.event_bus.publish(bus_key, {
                "type": "thinking_delta", "data": {"delta": delta},
            })
        ...
        def _on_token_bus(delta: str) -> None:
            ...
            try:
                ...
                loop.create_task(self.event_bus.publish(bus_key, {
                    "type": "token",
                    "data": {"delta": delta},
                }))
            except RuntimeError:
        ...
        def _track_out_token(count: int) -> None:
            ...
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(self.event_bus.publish(bus_key, {
                    "type": "out_token", "data": {"count": count},
                }))
            except RuntimeError:
                pass
        ...
        def _track_in_token(count: int) -> None:
            ...
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(self.event_bus.publish(bus_key, {
                    "type": "in_token", "data": {"count": count},
                }))
            except RuntimeError:
                pass
        ...
        async def _on_hook_event(hook_event: Any) -> None:
            ...
            await self.event_bus.publish(bus_key, {"type": "hook", "data": hook_data})
        ...
        # Invariant: the deltas streamed live via `in_token`/`out_token`
        # SSE events must sum to the final provider-billed totals that
        # go out in the `result` event. Mismatch = either a stream delta
        # was lost or `result` is hallucinating. Logged at WARNING so any
        # drift surfaces in the daemon log.
        try:
        ...
        # Workspace status (git info) — MUST run off the event loop.
        # `_get_workspace_status` makes 2–3 `subprocess.run(timeout=3-5s)`
        # calls, which blocked Uvicorn for seconds per turn → all other
        # sessions got `ConnectionRefused`. Push it to a thread.
        try:
            _ws = workspace or ""
            if _ws:
                from digitorn.core.api.apps import _get_workspace_status
        ...
        async def _publish(request: Any) -> None:
            try:
                ...
                await self.event_bus.emit(SessionEvent.build(
                    ...
                    app_id=app_id,
                    session_id=sid or "anonymous_session",
                    user_id=uid,
                    op_id=op_id,
    ...
    async def get_session(self, app_id: str, session_id: str, user_id: str | None = None) -> ConversationSession | None:
        """Get a conversation session by app + session ID.

        SECURITY: user_id is enforced. If user_id is None, falls back to
        "local" (single-user mode). Cross-user session scan is NOT allowed.
        """
        uid = user_id or "local"
        return await asyncio.to_thread(self._session_store.get, app_id, session_id, user_id=uid)
    ...
    async def list_sessions(
        ...
        if user_id:
            rows = await asyncio.to_thread(
                ...
                app_id, user_id, limit=limit, offset=offset,
            )
        else:
            rows = await asyncio.to_thread(
                self._session_store.list_for_app,
                app_id, limit=limit, offset=offset,
            )
    ...
    async def count_sessions(self, app_id: str, user_id: str | None = None) -> int:
                    ...
                    async with session.begin():
                        ...
                        if delete_history:
                            ...
                            await session.execute(
                                ...
                                {**params, "t": now, "r": "preserved_after_delete"},
                            )
    ...
    async def reload_from_db(self) -> list[str]:
        ...
        for app_row in apps:
            ...
            try:
                ...
                logger.error("Failed to reload '%s': %s", app_id, exc, exc_info=True)
