...
# Silence HuggingFace Hub + FastEmbed progress bars before any importer
# of tqdm in the chain (fastembed, sentence-transformers, transformers).
...
def create_app(settings: "Settings | None" = None) -> "socketio.ASGIApp":
    ...
    # (class-level since the module is `isolation=shared`).
    try:
    ...
    # In-progress ops registry, wired into the session event bus.
    # Non-fatal on failure: emit() falls back to no tracking.
    try:
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
        # Stamp the process-wide instance id so clients detect
        # restarts and wipe stale local state.
        try:
        ...
        # Boot-phase timings expose which step eats the budget.
        # Background tasks run in parallel and are not counted.
        _t0 = time.monotonic()
        ...
        # logs every 10s so a long cold start does not look like a hang.
        ...
        try:
            ...
            # Mirror SessionStore internal seq allocations into the
            # EventBuffer so wire emits never collide with them.
        ...
        # Warm tiktoken off-loop so the first turn doesn't trip the
        # slow-callback threshold loading cl100k_base.
        ...
        try:
            ...
            # inbox. Quota / usage tracking lives in the gateway.
            try:
                ...
                # DB url present → Postgres store; absent → file adapter
                ...
                # Notification dispatcher; degrades gracefully when
                # firebase-admin / SMTP creds are absent.
                ...
                # Daily prune of archived items older than 30 days; the
                # API only soft-deletes so unbounded growth is real.
                async def _prune_loop(store):
                    # Delay the first prune by 24 h so it doesn't compete
                    # with create_all + migrations at boot.
                    sleep_secs = 24 * 3600
                    try:
        ...
        # Wire the PackageRegistry before reloading deployed apps so
        # `_resolve_install_dir` can return the package's on-disk path.
        try:
        ...
        # Fire-and-forget reload_from_db so uvicorn starts accepting
        # HTTP immediately. `app.state.warming_up` exposes progress.
        ...
        async def _run_reload_from_db_bg() -> None:
            try:
                ...
                # Expected during shutdown; let the cancel propagate.
                raise
            except Exception as exc:
        ...
        if os.environ.get("DIGITORN_SKIP_BUILTINS"):
            ...
            async def _run_bootstrap_in_background():
                try:
                    # Wait for workers to be ready (same race fix as
                    # `_run_reload_from_db_bg`).
                    wl = None
        ...
        # Preload Whisper off-loop so the first /api/transcribe is instant.
        ...
        async def _session_evictor() -> None:
            while not app.state._shutting_down:
                ...
                try:
                    ...
                    logger.debug("server best-effort block failed: %s", exc)
        ...
        async def _activation_sweeper() -> None:
            """Mark zombie activations (`status=running` past timeout) as failed"""
            try:
            ...
            async def _sweep_iteration() -> None:
                ...
                # Silent-rot detector: find background-only apps whose
                # recent activation window is 100% failure.
                try:
        ...
        # SessionStore drain happens below via shutdown_session_store().
    ...
    # Hard cap on request body size to prevent DoS via oversized payloads.
    _MAX_BODY_BYTES = 16 * 1024 * 1024
    # Upload endpoints legitimately move more bytes; /messages allows up
    # to 25 MiB of base64-inflated attachments per POST.
    ...
    async def rate_limit_middleware(request: Request, call_next):
        ...
        # Only attack-surface paths get a rate-limit bucket (auth, admin,
        # messages, deploy, mcp, modules). Everything else is unthrottled.
    ...
    # `/api/packages` is gone - all install lifecycle routes live
    # under `/api/apps/*` via `apps_install_router`.
    ...
    async def api_admin_sessionstore_loadtest(payload: dict) -> dict:
        """Capacity probe: writes `sessions x events_per_session` events"""
    ...
    async def api_metrics_session_store() -> dict:
        """Hot-path health for the in-memory SessionStore."""
        from digitorn.core.runtime.session_store.bridge import (
            get_default_bridge,
        )
        bridge = get_default_bridge()
