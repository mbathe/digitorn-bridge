...
class ShoppingCart:
    ...
    def calculate_discount(self) -> float:
        ...
        for item in self.items:
            if self.coupon.excludes_digital and item.is_digital:
                continue
            if self.coupon.excludes_giftcards and item.is_giftcard:
                continue
            eligible += item.quantity * item.unit_price
        if eligible < self.coupon.min_cart_value:
            return 0.0
        discount = (eligible * self.coupon.percent_off / 100) + self.coupon.fixed_off
