...
|----------|-------|--------|
| Module id | `agent_spawn` | `module.py` |
...
- **Module sharing** — `memory`, `web`, `lsp`, `filesystem`,
  ...
  Other modules get fresh instances. (`runner.py:177`.)
...
→ {agent_id, status: "running", started_at}
```
...
→ {agent_id, status, duration_seconds, tool_calls_count, preview}
```
...
- App-config block reference (`agents` + `agents[].pool`):
  [Agents](../../app-language/03-agents.md)
- Multi-agent example:
  [Examples → Multi-Agent App](../../app-language/15-examples.md#8--multi-agent-coordinator--worker)
