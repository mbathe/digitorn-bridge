...
`security.behavior` (`schema.py:2509`) — under the canonical
`security:` block:
...
[Behavior Engine](../../app-language/43-behavior.md). Quick
recap below.

## Built-in profiles

`profiles.py:5-156`. 6 presets shipped:
...
`generic_rules.py:101-253`. Override any by adding a
...
`generic_rules.py:358-481` — 13 primitives plus `all` /
`any` / `not` composites.
...
`ClassifierConfig` (`schema.py:333`). Optional small LLM that
...
- Full YAML reference + condition primitives + examples by
  domain: [Behavior Engine](../../app-language/43-behavior.md)
- App-config block reference (`security.behavior`):
  [App Configuration → security](../../app-language/02-app-config.md#security--policy-only)
