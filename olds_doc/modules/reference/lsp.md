...
|----------|-------|--------|
| Module id | `lsp` | `module.py` |
| Version | `3.0.0` | `module.py` |
| Action count | 5 (all internal) | `module.py` |
...
- **REST endpoints** — the daemon exposes
  `POST /api/apps/{id}/sessions/{sid}/lsp/rpc` (raw LSP
  pass-through) and `POST .../lsp/cancel` for IDE-style
  integrations. See [API Integration → LSP](../../app-language/14-api-integration.md#lsp-language-server-proxy).
...
- LSP REST endpoints:
  [API Integration → LSP](../../app-language/14-api-integration.md#lsp-language-server-proxy)
