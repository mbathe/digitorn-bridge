...
|-----------|--------|--------|----------|
| Parallel execution | `run_parallel` | `context_builder/actions_meta.py:406` | always |
| Background launch (one action, 5 modes) | `background_run` | `context_builder/actions_background.py:164` | always |
| Persistent watcher start | `watch_start` | `context_builder/actions_watchers.py:99` | `runtime.watchers: true` |
| Watcher control | `watch_stop`, `watch_pause`, `watch_resume`, `watch_status`, `watch_list`, `watch_history` | `context_builder/actions_watchers.py:209-387` | `runtime.watchers: true` |
| Schedule a tool call | `schedule` | `cron_native/module.py:208` | `runtime.scheduler: true` |
| Cancel a scheduled job | `cancel_schedule` | `cron_native/module.py:437` | `runtime.scheduler: true` |
| Self-reminder | `remind` | `cron_native/module.py:523` | `runtime.scheduler: true` |
| Long-term memory write | `memory.remember` | `memory/module.py:491` | `tools.modules.memory` |
| Send via output channel | `tools.channels.send_message` | `channels/module.py:576` | `tools.modules.channels` + a configured channel |

## `runtime` flags that gate the primitives

Two flags on `RuntimeBlock` (`schema.py:2317`) gate the
...
runtime:
  ...
  scheduler: true      # enables schedule / cancel_schedule / remind
                       # (REQUIRES watchers: true; schema.py:2434)
```
...
`actions_meta.py:406`. Runs N actions concurrently via
...
**Params** (`RunParallelParams`, `params.py:115`):
...
`actions_background.py:164`. **A single `@action`** with five modes
...
**Params** (`BackgroundRunParams`, `params.py:137`):
...
Short alias in `tool_names.py:44` — `BackgroundRun` →
...
`actions_watchers.py:99`. Starts a periodic check.

**Params** (`WatchStartParams`, `params.py:173`):
...
`notify_when` strategies (`params.py:213-222`):
...
|--------|--------|--------|
| `watch_stop` | `actions_watchers.py:209` | `{ watcher_id }` |
| `watch_pause` | `actions_watchers.py:253` | `{ watcher_id }` |
| `watch_resume` | `actions_watchers.py:284` | `{ watcher_id }` |
| `watch_status` | `actions_watchers.py:318` | `{ watcher_id }` |
| `watch_list` | `actions_watchers.py:346` | (none) |
| `watch_history` | `actions_watchers.py:387` | `{ watcher_id, last_n }` (last_n: 1-100, default 10) |
...
agent. The scheduler is daemon-backed (`packages/digitorn/modules/cron_native/`)
and survives daemon restarts via persisted state.

> **Important.** The scheduler block requires `runtime.watchers: true`
> as well (`schema.py:2434`); the watcher loop is the heartbeat the
> scheduler runs on.

### `schedule`

`cron_native/module.py:208`. The single primitive that schedules
anything — one-shot or recurring.

**Params** (`ScheduleParams`, `cron_native/params.py:10`):
...
Three accepted shapes (`cron_native/params.py:21-37`):
...
{"name": "schedule", "arguments": {
  "when": "0 9 * * 1-5",
  "action": "http.get",
  "args": {"url": "https://api.internal/health"},
...
`cron_native/module.py:437`.

**Params** (`CancelScheduleParams`, `cron_native/params.py:93`):
...
`cron_native/module.py:523`. A self-prompt scheduled back into the
current session.

**Params** (`RemindParams`, `cron_native/params.py:115`):
...
`memory/module.py:491`. Stores a fact that **survives compaction**.
...
(`memory/module.py:341, 378, 439, 458`): `task_create`,
`task_update`, `set_goal`, `remember`. Short aliases (from
`tool_names.py:32-34`): `TaskCreate`, `TaskUpdate`, `Remember`.
...
(`packages/digitorn/modules/channels/module.py:576`).
...
The full `channels` action set (`channels/module.py:566+`) :
...
> Pydantic model in `context_builder/params.py:266` is currently
...
- Source of truth for short ↔ FQN mapping: `tool_names.py:21-71`
