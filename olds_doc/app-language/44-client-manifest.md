...
(coordinator, multi-agent research, dev assistants) opt in.

The pane is driven by the daemon-resource protocol: snapshot on
mount + Socket.IO live updates + heartbeat-driven reconcile, so it
survives daemon restarts, socket drops, and tab-focus cycles
without zombie state. See [Daemon resource protocol E2E](
../daemon-resource-protocol-e2e.md) for the wire contract.
...
| Token | Type | Source |
| ----- | ---- | ------ |
| `{{tool.name}}` | str | Short tool name (`WsRead`, `Bash`, …) |
| `{{tool.params.X}}` | any | Individual params (the daemon's args) |
| `{{tool.result.X}}` | any | Fields from the result payload |
| `{{tool.result}}` | json | JSON dump of the whole result |
| `{{tool.error}}` | str | Empty string when `status == success` |
| `{{tool.duration_ms}}` | int | `completedAt - startedAt` (ms) |
| `{{tool.status}}` | str | `running` \| `success` \| `error` |
...
**Rollback.** The block lives in
`packages/digitorn/core/app/tool_renderers.py`. Removing the file
...
in [`tool-call-block.tsx`](
../../digitorn_web/src/components/chat/tool-call-block.tsx); Flutter
has a single `_renderToolEntry` method in [`chat_bubbles.dart`](
../../digitorn_client/lib/ui/chat/chat_bubbles.dart). Replacing those
