...
`Settings` Pydantic model in
`packages/digitorn/core/config.py:760`. Settings load from layered
...
`Settings` (`config.py:760`) groups configuration into 19 nested
sub-models:

| Block | Pydantic class | Source line | Covers |
|-------|----------------|-------------|--------|
| `server` | `ServerConfig` | `config.py:25` | FastAPI / Uvicorn — host, port, workers, CORS, TLS, sandbox toggle. |
| `database` | `DatabaseConfig` | `config.py:163` | Storage backend (SQLite / Postgres). |
| `modules` | `ModulesConfig` | `config.py:188` | Module loading toggles + per-module config. |
| `logging` | `LoggingConfig` | `config.py:658` | Log level + format. |
| `app` | `AppConfig` | `config.py:215` | Built-in app behavior (digitorn-chat default app). |
| `runtime` | `RuntimeConfig` | `config.py:239` | Daemon-wide runtime defaults — context window, timeouts. |
| `default_model` | `DefaultModelConfig` | `config.py:620` | Default LLM provider/model used by built-in apps when none is set. |
| `auth` | `AuthConfig` | `config.py:280` | JWT / API-key authentication. |
| `oauth` | `OAuthConfig` | `config.py:130` | OAuth providers (Google, Microsoft, GitHub, ...). |
| `session` | `SessionConfig` | `config.py:357` | Session lifecycle defaults. |
| `agent_spawn` | `AgentSpawnConfig` | `config.py:478` | Defaults for the agent_spawn module (max_workers, timeouts). |
| `mcp` | `MCPConfig` | `config.py:499` | MCP server connection pool defaults. |
| `sandbox` | `SandboxConfig` (daemon-side) | `config.py:520` | Per-app OS sandbox enforcement. |
| `websocket` | `WebSocketConfig` | `config.py:541` | Socket.IO / WS server tuning. |
| `discovery` | `DiscoveryConfig` | `config.py:591` | Tool discovery (semantic + keyword index). |
| `images` | `ImageConfig` | `config.py:554` | Multimodal image handling (max size, aging policy). |
| `transcribe` | `TranscribeConfig` | `config.py:665` | Voice transcription (model selection, fallback). |
| `hub` | `HubConfig` | `config.py:739` | Hub registry integration. |
| `session_queue` | `SessionQueueConfig` | `config.py:408` | Background-mode activation queue. |

> **Note**. The daemon's `sandbox` sub-block (`config.py:520`) is
> the OS sandbox toggle and pool size for **all** deployed apps —
> distinct from `security.sandbox` in an app's YAML
> (`schema.py:1399`), which is the per-app sandbox declaration.
> Both work together: the daemon enables sandboxing globally, the
> app declares the level it wants.

## `server:` — FastAPI / Uvicorn

`config.py:25` `ServerConfig`. Default values are production-safe
...
| `cors_origins` | list[string] | `[localhost:*, app.digitorn.ai, api.digitorn.ai]` | Allowed CORS origins. **Wildcard `*` is rejected** (validator at `config.py:108`). |
...
`config.py:280` `AuthConfig`. The full surface is documented in
...
- `mode` — must be `"remote"` when `auth_enabled: true`. The legacy
  `"embedded"` mode is rejected at startup (`server.py:1374-1389`).
...
`config.py:130` `OAuthConfig`. Each sub-section
(`OAuthProviderConfig`, `config.py:119`) is `{client_id,
...
`public_base_url` (default `http://localhost:8000`) is used to
build the OAuth callback URL (`<base>/auth/oauth/<provider>/callback`).
Set it to your daemon's externally-reachable URL in prod.

## `database:` — Storage

`config.py:163` `DatabaseConfig`. SQLite by default; Postgres for
...
`config.py:620` `DefaultModelConfig`. Used by the auto-deployed
`digitorn-chat` and other builtins when no brain is configured.
...
`config.py:357` `SessionConfig` and `config.py:408`
...
`config.py:554` `ImageConfig`. Image-aging policy applied across
...
`config.py:591` `DiscoveryConfig`. Controls the FastEmbed +
...
`config.py:499` `MCPConfig`. Connection-pool defaults shared
...
`config.py:739` `HubConfig`. Connect the daemon to the public
...
|-------|-----------------|--------|
| `server.host` | `127.0.0.1` | user YAML |
| `server.port` | `9000` | user YAML overrides system |
| `server.workers` | `4` | env (no YAML override) |
| `auth.service_url` | `https://auth.shared.example.com` | system YAML — **YAML beats env** |
