...
`SecurityBlock.behavior` (`schema.py:2509`) — the `behavior:`
...
`BehaviorConfig` (`schema.py:479`) has `extra: forbid` — typos
fail the compiler.
...
`schema.py:479-552`. All optional, all `extra: forbid`.
...
`profiles.py:5-156`. Six presets, each a dict of toggles +
numeric thresholds:
...
`BehaviorRuleDefinition` (`schema.py:149`, `extra: forbid`).
...
### Trigger matching (`generic_rules.py:263`)
...
### Conditions (`generic_rules.py:358-481`)
...
fire (`generic_rules.py:480`). Forward-compatible — an old
daemon never accidentally enforces a rule it doesn't understand.

### Message templates (`generic_rules.py:486-553`)
...
### Counters (`StateTrackingCounterConfig` `schema.py:233`)
...
> (`schema.py:245-252`) — counters that never tick are
> rejected at compile time.

### Flags (`StateTrackingFlagConfig` `schema.py:255`)
...
> validation error (`schema.py:263-269`) — same logic as
> counters.

## The 14 built-in rules

`generic_rules.py:101-253` `DEFAULT_RULE_DEFINITIONS`. Each
...
`ClassifierConfig` (`schema.py:333`, `extra: forbid`). A small
...
`brain` (`schema.py:537`) picks a separate LLM. When `null`,
