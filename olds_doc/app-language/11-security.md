...
|-------|--------|-------|------------------|
| `tools.capabilities` | `schema.py:554` `CapabilitiesConfig` | Application — runs in-process before every tool call | Which actions the agent can call, with grant / approve / deny / risk gates. |
| `security.behavior` | `schema.py:479` `BehaviorConfig` | Behavioural — declarative rules + classifier injected into the agent loop | Pattern-based behaviour rules (read before edit, test after change, ...). |
| `security.sandbox` | `schema.py:1399` `SandboxConfig` | OS kernel — Landlock / seccomp / namespaces / Job Objects | Process-level isolation (filesystem, network, process spawning). |
...
`CapabilityGrant` (`schema.py:120`) is `{module: str, actions:
...
`security.py:286` `security_gate()`. Every tool call passes
...
| Gate | Code label | Triggers when... | Source |
|------|------------|-------------------|--------|
| 0 | `gate0_inactive` | The app is deployed but not active. Admin profiles bypass. | `security.py:329` |
| 1a | `gate1_module` | The agent's profile can't access the module (`hidden_modules` or per-agent `modules` restriction filters it out). | `security.py:344` |
| 1b | `gate1_hidden` | The action is in `hidden_actions` for this module. | `security.py:348` |
| 2 | `gate2_risk` | Action's declared risk exceeds `max_risk_level`, with no explicit grant or per-action policy. | `security.py:356` |
| 3 | `gate3_permissions` | Action declares `required_permissions` (symbolic, e.g. `fs.write`, `net.http`) and the profile lacks them. | `security.py:363` |
| 4 | `gate4_policy` | Resolved action policy is `block` (denied) or `approve` (paused for HITL). | `security.py:370, 375` |
| 5 | `gate5_classification` | Per-tool data-classification rule rejects the params (e.g. PII detected in a non-PII-allowed channel). | `security.py:388` |
| 6 | `gate6_rate_limit` | Per-action rate limit window exceeded. | `security.py:397` |

System modules (`context_builder`, `llm_provider`, `index`) bypass
the gates entirely — they're internal infrastructure, not
user-facing tools (`security.py:337`).

The infrastructure meta-actions (`execute_tool`, `search_tools`,
`get_tool`, `list_categories`, `browse_category`, `run_parallel`)
are also bypassed at the dispatcher level (`security.py:312`); the
...
in its decorator. `max_risk_level` (`schema.py:563`) caps what an
...
`schema.py:479` `BehaviorConfig` (`extra: forbid`). The behaviour
...
Defined under `packages/digitorn/modules/behavior/profiles.py`. Each
profile bundles a set of rules and sensible defaults:
...
`BehaviorCustomRule` (`schema.py:133`):
...
`schema.py:1399` `SandboxConfig` (`extra: forbid`). Optional. When
...
`schema.py:1419`. Four presets, each adding capabilities to the
previous one:
...
`packages/digitorn/core/security_audit.py`. Every gate decision
fires `log_security_event(...)` with:
...
- Per-module security knobs (filesystem path sandboxing, web egress
  ...
  [modules/index.md](../modules/index.md) and the per-module
