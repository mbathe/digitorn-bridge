...
`server.py:1471-1486` mounts the routers. Prefixes:

| Router | Prefix | Source |
|--------|--------|--------|
| Apps (lifecycle, sessions, messages, ...) | `/api/apps` | `apps_v2/__init__.py:113` |
| Apps install / upgrade / uninstall | `/api/apps` | `apps_install.py:65` |
| Discovery (modules, triggers, templates, compile) | `/api/discovery` | `discovery.py:41` |
| Credentials | `/api` (mixed paths) | `credentials.py` |
| MCP (search, install, pool, OAuth) | `/api/mcp` | `mcp.py` |
| Hub (marketplace) | `/api/hub` | `hub.py` |
| Auth | `/auth` (proxied) | `_auth_redirect_router` (`server.py:1455-1469`) |
| Health / metrics | `/health`, `/healthz`, `/readyz`, `/api/metrics`, `/metrics` | `server.py:1549-1674` |
...
(`server.py:146` `create_socketio_server`,
`socketio_bus.py:259`). Defaults
(`socketio_bus.py:242-244`):
...
5. Tool resumes (or returns `denied`). Default timeout
   `capabilities.approval_timeout` (`schema.py:579`,
...
| `POST` | `/api/apps/{app_id}/notifications` | `diag.py:856` | Drain pending notifications + trigger an agent turn to process them. |
...
`server.rate_limit_rpm` (`config.py:32`) — **100 000 RPM** by
default (effectively off). Admin / auth / deploy buckets get
half (`server.py:1253`). Per-app and per-user overrides:
...
| `POST` | `/api/discovery/compile` | `:748` | Compile a YAML body without deploying. |
| `POST` | `/api/discovery/prompt-preview` | `:899` | Resolve a system prompt (with bundle namespaces) for preview. |
| `POST` | `/api/discovery/generate-package-manifest` | `:998` | Walk a package dir, produce its manifest. |
...
[credentials.md](../credentials.md). Key endpoints:
...
`server.py:1549-1674`. Not scoped to an app:
...
| `GET`  | `/api/metrics/sessions` | Session-level metrics. |
| `GET`  | `/api/metrics/sessions/{sid}` | One session. |
| `GET`  | `/api/metrics/apps/{app_id}` | One app. |
| `GET`  | `/metrics` | Alias of `/api/metrics/prometheus`. |
