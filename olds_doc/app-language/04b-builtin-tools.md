...
|------|--------|-------------|
| `search_tools` | `actions_meta.py:117` | Hybrid (semantic + keyword) search over the agent's visible tool index. |
| `get_tool` | `actions_meta.py:159` | Full JSON schema + examples + side-effects + aliases for one tool. |
| `execute_tool` | `actions_meta.py:204` | Execute a tool by name with parameters. The agent loop also auto-routes direct calls (e.g. `filesystem.read({...})`) through this action. |
| `list_categories` | `actions_meta.py:312` | List all tool domains (modules) currently visible to this agent. |
| `browse_category` | `actions_meta.py:342` | Paginated list of tools in one domain. |
| `run_parallel` | `actions_meta.py:406` | Execute multiple tool calls concurrently via `asyncio.gather`. |
| `use_skill` | `actions_meta.py:740` | Invoke a `/command` skill from `dev.skills` or the bundle's `skills/` dir. |
| `call_app` | `actions_meta.py:776` | Call another deployed Digitorn app as a tool (composition pattern). |
| `ask_user` | `actions_meta.py:859` | Pause the loop and ask the user a typed question (HITL). |
| `background_run` | `actions_background.py:164` | Launch any tool as a background task. **One action, five modes** dispatched by params: launch (`name`+`params`), status (`task_id`), cancel (`task_id`+`cancel=true`), wait (`task_id`+`wait=true`+`timeout`), list (`list_tasks=true`). |

Short-name aliases registered in
`packages/digitorn/core/runtime/tool_names.py:21`:
...
| Tool | Source | Description |
|------|--------|-------------|
| `watch_start` | `actions_watchers.py:99` | Start a periodic watcher that runs a tool every N seconds and records the result. |
| `watch_stop` | `actions_watchers.py:209` | Stop a watcher (frees its slot). |
| `watch_pause` | `actions_watchers.py:253` | Pause a running watcher. |
| `watch_resume` | `actions_watchers.py:284` | Resume a paused watcher. |
| `watch_status` | `actions_watchers.py:318` | Get a single watcher's status, last result, run count, error rate. |
| `watch_list` | `actions_watchers.py:346` | List all watchers (active + paused). |
| `watch_history` | `actions_watchers.py:387` | Last N check results for one watcher. |
...
| Tool | Source | Description |
|------|--------|-------------|
| `schedule` | `cron_native/module.py:208` | Schedule a tool call. Accepts `at: <ISO timestamp>` (one-shot), `delay: <seconds>` (one-shot), or `cron: <expr>` (recurring). |
| `cancel_schedule` | `cron_native/module.py:437` | Cancel a scheduled job by id. |
| `remind` | `cron_native/module.py:523` | Convenience wrapper that schedules a "self-prompt" reminder back to the agent at a given time. |

`runtime.scheduler: true` requires `runtime.watchers: true`
(`schema.py:2434` `RuntimeBlock.scheduler`).
...
available
(`packages/digitorn/modules/memory/module.py:341, 378, 439, 458` —
all four `@action` decorators).

| Tool | Short alias | Source | Description |
|------|-------------|--------|-------------|
| `memory.task_create` | `TaskCreate` | `memory/module.py:369` | Create a task in the agent's working-memory todo list. |
| `memory.task_update` | `TaskUpdate` | `memory/module.py:403` | Update a task's status (`pending`, `in_progress`, `done`, `blocked`) or other fields. |
| `memory.set_goal` | — | `memory/module.py:450` | Set or update the agent's current high-level goal. |
| `memory.remember` | `Remember` | `memory/module.py:491` | Store a long-term fact that survives compaction. Searchable via the same module's recall layer at runtime. |

Short aliases come from `tool_names.py:32-34`. Anything else (set_plan,
...
`packages/digitorn/modules/agent_spawn/module.py:273` exposes a
...
Short alias: `Agent` → `agent_spawn.agent` (`tool_names.py:41`).
...
(`tool_names.py:46-51`) appear in the agent's index. They operate on
